          ...
          <div class="info-edit_card-info-warp">
            <div class="info-edit_card-info-warp-item" v-for="i in item.images">
              <div class="info-edit_card-info-warp-item-img">
                ...
                <span @click="removeImg(i,item.images)" class="info-edit_card-info-warp-item-img-remove">×</span>
              </div>
            </div>
            <div class="info-edit_card-info-warp-item">
              <a class="file">
                  <span>╋</span>
                  <input :ref="'checkFile_'+ index" type="file" accept="image" @change="uploadImage($event,item,index)">
              </a>
            </div>
            <div class="info-edit_card-info-warp">
...
export default {
  ...
  methods: {
    ...
    uploadImage(e,info,index){
      let files = e.target.files || e.dataTransfer.files;
      if (!files.length) return;
      let picValue = files[0];
      let domm = this.$refs['checkFile_'+index][0];
      imgPreview(picValue,this,this.postImg,domm,info.images);
    },
    // showCheckItem
    showCheckItem(index , item){
      let title = 'checkItem_'+ index;
      let sty = this.$refs[title][0].style.display;
      switch (sty) {
        case 'none':
          this.$refs[title][0].style.display="block";
          break;
        default:
          this.$refs[title][0].style.display="none";
          break;
      }
    },
  },
